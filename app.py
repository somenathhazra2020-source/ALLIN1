...
IMAGE_ROOT = (Path(__file__).resolve().parent / "images").resolve()

def _find_product_image(name, category):
    """Return an existing bundled PNG. Never return a non-existent path."""
    target = _slug_image(name)
    cat_slug = _slug_image(category or "")

    # 1) Exact product-name match inside its own category.
    category_dir = IMAGE_ROOT / cat_slug
    if category_dir.is_dir():
        matches = sorted(category_dir.glob(f"*_{target}.png"))
        if matches and matches[0].is_file():
            return matches[0]

    # 2) Exact product-name match anywhere (protects against category-name changes).
    for d in IMAGE_ROOT.iterdir() if IMAGE_ROOT.is_dir() else []:
        if d.is_dir():
            matches = sorted(d.glob(f"*_{target}.png"))
            if matches and matches[0].is_file():
                return matches[0]

    # 3) Safe category fallback. This prevents Streamlit Cloud from crashing
    # if an old SQLite database contains a product name that is not in the
    # current 600-item catalog.
    if category_dir.is_dir():
        category_images = sorted(category_dir.glob("*.png"))
        if category_images:
            return category_images[0]

    # 4) Any bundled image as the last-resort safety fallback.
    for f in IMAGE_ROOT.rglob("*.png") if IMAGE_ROOT.is_dir() else []:
        if f.is_file():
            return f
    return None
...
def product_image(name, category):
    path = _find_product_image(name, category)
    return str(path) if path else None

def show_product_image(image_path, alt_text):
    """Render an image safely; a missing/corrupt asset can never crash the app."""
    if image_path:
        try:
            p = Path(image_path)
            ...
            pass
    st.markdown(
        f'<div class="bm-image-fallback">🛍️<br><span>{alt_text}</span></div>',
...
def product_card(uid,x,i,prefix='shop',compact=False):
    ...
    if compact:
        with st.container(border=True):
            ...
            show_product_image(image_path, x["name"])
